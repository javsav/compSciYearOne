...
% with rows 3 to 255, where 255 represents the colour white, and 0 
% represents black. The image is darker on the top and lighter on the bottom.
% It then displays the image and saves it as a file called test1.png
...
for row = [1:numRows] % Iterate through 1 to numRows
    ...
    for col = [1:numCols] % For each row in 1:numRows, iterate through 1:numCols
        pixelVal = uint8((row/imSize) * whitePix); % Calculate a pixel value for each row in range 3:255, each row increases by 2 or 3
