...
% %                 )
% %             end
            for character = 1:length(A{item}(end))
                %if A{item(character)} == length(A{item(end)})
                    fprintf('%0.f, ', A{item(character)}) % If item is the final index of A do not print the comma
