...
while number == 0  || isa(number, 'char')
    number = input("Please enter a numerical value other than zero: ");
end

if mod(number, 4) == 0
    factorTwo = number/4;
    fprintf("%0.1f is divisible by four;\n %i * 4 = %0.1f. \n", number, factorTwo, number)
else
    fprintf("%0.f is not divisible by four, 4 / %0.f leaves a remainder of %0.2f", number, number, mod(number, 4))
end