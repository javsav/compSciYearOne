...
% with rows 255 to 3, where 255 represents the colour white, and 0 
% represents black. The image is lighter on the top and darker on the bottom.
% It then displays the image and saves it as a file called test2.png
...
for row = [1:numRows] % Iterate through 1 to numRows
    ...
    for col = [1:numCols] % For each row in 1:numRows, iterate through 1:numCols
        pixelVal = uint8((imSize - row + 1)/imSize * whitePix); % Calculate a pixel value for each row in range 255:3, each row decreases by 2 or 3
